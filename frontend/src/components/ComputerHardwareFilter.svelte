<script>
    ...
    async function fetchFilterOptions() {
        try {
            const response = await fetch('http://localhost:3001/api/filter-com-hardware');
            ...
            if (preselectedSubCategory) {
                const subCategoryId = parseInt(preselectedSubCategory);
                const isComponent = filterData.categories.components.some(cat => cat.id === subCategoryId);
                const isPeripheral = filterData.categories.peripherals.some(cat => cat.id === subCategoryId);
                if (isComponent || isPeripheral) {
                    selectedSubCategories = [subCategoryId];
                    showCategories = true;
                    showComponents = isComponent;
                    showPeripherals = isPeripheral;
                    applyFilters();
                }
            }
        } catch (err) {
    ...
    const toggleSubCategory = (id) => {
        selectedSubCategories = selectedSubCategories.includes(id)
            ? selectedSubCategories.filter(c => c !== id)
    ...
    async function applyFilters() {
        try {
            ...
            queryParams.append('minPrice', Number(minPrice).toString());
            queryParams.append('maxPrice', Number(maxPrice).toString());
            ...
            const response = await fetch(`http://localhost:3001/api/filter-com-hardware/products?${queryParams.toString()}`);
    ...
    function resetFilters() {
        selectedSubCategories = preselectedSubCategory ? [parseInt(preselectedSubCategory)] : [];
        selectedBrands = [];
        filterData.availability.inStock = false;
        filterData.availability.outOfStock = false;
        minPrice = filterData.priceRange.min.toString();
        maxPrice = filterData.priceRange.max.toString();
        applyFilters();
    }
</script>
...
<div class="filter-container flex flex-col gap-4 p-4">
    ...
    {:else}
        ...
        <!-- Categories Section with Nested Dropdowns -->
        <div class="filter-section">
            <button
                class="flex justify-between items-center w-full text-left py-2"
                on:click={() => showCategories = !showCategories}
            >
                <h2 class="text-lg text-[#CA9335] font-semibold">Categories ({selectedSubCategories.length})</h2>
            ...
            {#if showCategories}
                <div class="flex flex-col gap-2 pl-4 pb-2" transition:fade>
                    ...
                    <div>
                        <button
                            class="flex justify-between items-center w-full text-left py-2"
                            on:click={() => showComponents = !showComponents}
                        >
                            <h3 class="text-sm text-[#CA9335] font-semibold">Components</h3>
                            <span class="text-[#CA9335] text-lg">{showComponents ? '−' : '+'}</span>
                        </button>
                        {#if showComponents}
                            <div class="flex flex-col gap-2 pl-4 pb-2" transition:fade>
                                {#each filterData.categories.components as component}
                                    <div class="flex items-center gap-2">
                                        <input 
                                            type="checkbox" 
                                            id={`comp-${component.id}`}
                                            checked={selectedSubCategories.includes(component.id)}
                                            on:change={() => toggleSubCategory(component.id)}
                                        >
                                        <label class="text-sm text-gray-600 font-semibold" for={`comp-${component.id}`}>{component.name}</label>
                                    </div>
                                {/each}
                            </div>
                        {/if}
                    ...
                    <div>
                        <button
                            class="flex justify-between items-center w-full text-left py-2"
                            on:click={() => showPeripherals = !showPeripherals}
                        >
                            <h3 class="text-sm text-[#CA9335] font-semibold">Peripherals</h3>
                            <span class="text-[#CA9335] text-lg">{showPeripherals ? '−' : '+'}</span>
                        </button>
                        {#if showPeripherals}
                            <div class="flex flex-col gap-2 pl-4 pb-2" transition:fade>
                                {#each filterData.categories.peripherals as peripheral}
                                    <div class="flex items-center gap-2">
                                        <input 
                                            type="checkbox" 
                                            id={`peri-${peripheral.id}`}
                                            checked={selectedSubCategories.includes(peripheral.id)}
                                            on:change={() => toggleSubCategory(peripheral.id)}
                                        >
                                        <label class="text-sm text-gray-600 font-semibold" for={`peri-${peripheral.id}`}>{peripheral.name}</label>
                                    </div>
                                {/each}
                            </div>
                        {/if}
